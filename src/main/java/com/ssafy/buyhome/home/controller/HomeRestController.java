...
public class HomeRestController {
    ...
    @GetMapping("/aptimage/{aptName}")
    public ResponseEntity<?> getAptImage(@PathVariable("aptName") String aptName) {
        String clientId = "JodwiVXhYmXk3rD8cc5O"; //애플리케이션 클라이언트 아이디
        String clientSecret = "aoJ7ssMour"; //애플리케이션 클라이언트 시크릿


        String text = null;
        try {
            text = URLEncoder.encode(aptName, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            throw new RuntimeException("검색어 인코딩 실패",e);
        }


        String apiURL = "https://openapi.naver.com/v1/search/image?query="+text+"%EC%A3%BC%EC%8B%9D&display=1&start=1&sort=sim";    // JSON 결과
